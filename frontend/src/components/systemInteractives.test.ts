...
function moved(before: Record<string, string>, after: Record<string, string>): number {
...
describe('consistent hashing', () => {
  ...
  it('keys that move on a join move only to the new node — no key is shuffled between old ones', () => {
    ...
    for (const key of RING_KEYS) {
      if (before[key] !== after[key]) expect(after[key]).toBe('e');
    }
  });

  it('virtual nodes even out a distribution that a single point per node leaves lumpy', () => {
    const share = (virtualNodes: number) => {
      const map = assign(['a', 'b', 'c', 'd'], virtualNodes, 'ring');
      const counts = new Map<string, number>();
      for (const key of RING_KEYS) counts.set(map[key], (counts.get(map[key]) ?? 0) + 1);
...
describe('bloom filter', () => {
  const insert = (bits: Uint8Array, value: string, k: number) => {
    for (const index of bloomIndices(value, k)) bits[index] = 1;
  };
  const probe = (bits: Uint8Array, value: string, k: number) =>
    bloomIndices(value, k).every((index) => bits[index] === 1);
  ...
  it('never reports a false negative — the guarantee the whole structure rests on', () => {
    for (const k of [1, 2, 3, 4, 5, 6]) {
      ...
      for (const value of members) insert(bits, value, k);
      for (const value of members) expect(probe(bits, value, k)).toBe(true);
    }
  });
  ...
  it('false positives appear as the array fills, and the measured rate tracks the formula', () => {
    ...
    for (let index = 0; index < 4000; index++) {
      ...
      trials++;
      if (probe(bits, candidate, k)) positives++;
    }
