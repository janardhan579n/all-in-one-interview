import type * as BundleModule from './bundle';
 ...
 *
 * It talks to the Spring Boot API when the backend is reachable and falls back to the bundled
 * content plus localStorage when it is not (ADR-006). Pages never know which mode they are in;
 * they call `platform.lessons()` and get the same shape either way.
 *
 * The mode is probed once at startup and surfaced in the top bar, because silently degrading
 * is worse than degrading loudly.
 */
 ...
 */
let bundle: typeof BundleModule = undefined as unknown as typeof BundleModule;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const healthy =
        body !== null &&
        typeof body === 'object' &&
        (body as { status?: unknown }).status === 'UP';
...
export async function stats(): Promise<Record<string, number>> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<Record<string, number>>('/meta/stats');
  return bundle.stats as unknown as Record<string, number>;
}
...
export async function lessons(filters: LessonFilters = {}): Promise<ContentSummary[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    ...
    return request<ContentSummary[]>(withLanguage(`/dsa/lessons${suffix ? `?${suffix}` : ''}`));
  }
...
export async function lesson(id: string): Promise<Lesson> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<Lesson>(withLanguage(`/dsa/lessons/${id}`));
  const found = bundle.lessons.get(id);
  if (!found) throw new Error(`No lesson with id '${id}'`);
  return bundle.localise(found, language);
}

export async function dsaGroups(): Promise<{ group: string; lessons: ContentSummary[] }[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<{ group: string; lessons: ContentSummary[] }[]>('/dsa/groups');

  const byGroup = new Map<string, ContentSummary[]>();
...
export async function patternMap(): Promise<PatternMap> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<PatternMap>('/dsa/pattern-map');
  if (!bundle.patternMap) throw new Error('Pattern map is missing from the content bundle');
...
export async function problems(filters: ProblemFilters = {}): Promise<ContentSummary[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    const query = new URLSearchParams();
    Object.entries(filters).forEach(([key, value]) => {
    ...
    const suffix = query.toString();
    return request<ContentSummary[]>(`/problems${suffix ? `?${suffix}` : ''}`);
  }
...
export async function problem(id: string): Promise<Problem> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<Problem>(`/problems/${id}`);
...
export interface PatternGroup {
  ...
  patternTitle: string;
  group: string;
  tiers: { difficulty: string; problems: ContentSummary[] }[];
}

export async function problemsByPattern(): Promise<PatternGroup[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<PatternGroup[]>('/problems/by-pattern');

  const grouped = new Map<string, Map<string, ContentSummary[]>>();
  for (const item of bundle.problems.values()) {
    const patternId = item.patternId ?? 'unclassified';
    const tier = grouped.get(patternId) ?? new Map<string, ContentSummary[]>();
  ...
  return [...grouped.entries()].map(([patternId, tiers]) => {
    ...
    return {
      ...
      tiers: DIFFICULTY_ORDER.filter((difficulty) => (tiers.get(difficulty) ?? []).length > 0).map((difficulty) => ({
        difficulty,
        problems: tiers.get(difficulty) as ContentSummary[],
      })),
    };
  });
}

export async function dailyProblem(): Promise<ContentSummary> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<ContentSummary>('/problems/daily');
...
export async function concepts(group?: string): Promise<ContentSummary[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    return request<ContentSummary[]>(`/system-design/concepts${group ? `?group=${encodeURIComponent(group)}` : ''}`);
  }
...
export async function concept(id: string): Promise<Concept> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<Concept>(withLanguage(`/system-design/concepts/${id}`));
  const found = bundle.concepts.get(id);
  if (!found) throw new Error(`No concept with id '${id}'`);
  return bundle.localise(found, language);
}

export async function caseStudies(): Promise<ContentSummary[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<ContentSummary[]>('/system-design/case-studies');
  return [...bundle.caseStudies.values()].map((study) => bundle.summarise(study, 'case-study'));
}

export async function caseStudy(id: string): Promise<CaseStudy> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<CaseStudy>(withLanguage(`/system-design/case-studies/${id}`));
  const found = bundle.caseStudies.get(id);
  if (!found) throw new Error(`No case study with id '${id}'`);
  return bundle.localise(found, language);
}

export async function decisionTree(id: string): Promise<DecisionTree> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<DecisionTree>(`/decision-trees/${id}`);
  const found = bundle.decisionTrees.get(id);
  if (!found) throw new Error(`No decision tree with id '${id}'`);
  return found;
}

// --------------------------------------------------------------- learning paths

export async function paths(): Promise<{ id: string; title: string; audience: string; description: string; steps: number }[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    return request<{ id: string; title: string; audience: string; description: string; steps: number }[]>(
      '/learning/paths',
    );
  }
...
export async function path(id: string): Promise<ResolvedPath> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<ResolvedPath>(`/learning/paths/${id}`);

  const found = bundle.paths.get(id) as LearningPath | undefined;
  if (!found) throw new Error(`No learning path with id '${id}'`);

  const progress = local.snapshot();
  const steps = found.steps.map((step) => {
    const document = bundle.anyById(step.ref);
    const record = document as unknown as Record<string, unknown> | undefined;
    return {
      ref: step.ref,
      kind: step.kind,
      title: document?.title ?? step.ref,
      summary: (record?.summary as string) ?? '',
      estimatedMinutes: (record?.estimatedMinutes as number) ?? 0,
      status: (progress.lessons[step.ref]?.status ?? 'not_started') as ProgressStatus,
      missing: !document,
    };
  });
...
export async function nextUp(): Promise<{ lessonId: string; type?: string; title?: string; summary?: string; reason: string }> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    return request<{ lessonId: string; type?: string; title?: string; summary?: string; reason: string }>(
      '/learning/next',
    );
  }
    ...
    if (row.status === 'in_progress') {
      ...
      if (document) {
        return {
          ...
          summary: (document as unknown as Record<string, string>).summary ?? '',
...
export async function progress(): Promise<ProgressSnapshot> {
  if (mode === 'online') return request<ProgressSnapshot>('/progress');
  return local.snapshot();
}

export async function progressSummary(): Promise<ProgressSummary> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<ProgressSummary>('/progress/summary');

  const snapshot = local.snapshot();
  const store = local.read();

  const completedOf = (type: 'lesson' | 'concept' | 'case-study') =>
    Object.entries(snapshot.lessons).filter(
      ([id, row]) => row.status === 'completed' && bundle.typeOf(id) === type,
    ).length;
  ...
  const patternsDone = Object.entries(snapshot.lessons).filter(
    ([id, row]) => row.status === 'completed' && bundle.lessons.get(id)?.kind === 'pattern',
  ).length;

  const solved = new Set(store.attempts.filter((attempt) => attempt.solved).map((attempt) => attempt.problemId)).size;

  const byPattern = new Map<string, { attempts: number; solved: number }>();
  for (const attempt of store.attempts) {
    if (!attempt.patternId) continue;
  ...
  const band =
    score >= 80 ? 'Interview ready' : score >= 55 ? 'Nearly there' : score >= 25 ? 'Building foundations' : 'Just getting started';
...
export async function updateLessonProgress(contentId: string, status: ProgressStatus, percent?: number): Promise<void> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    await request<void>(`/progress/lessons/${contentId}`, {
      method: 'PUT',
...
export async function recordAttempt(problemId: string, solved: boolean, confidence?: number): Promise<void> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    await request<void>(`/progress/problems/${problemId}/attempt`, {
      method: 'POST',
...
export async function setFlags(contentId: string, difficult: boolean, mastered: boolean): Promise<void> {
  if (mode === 'online') {
    await request<void>(`/progress/flags/${contentId}`, {
      method: 'PUT',
...
export async function quizQuestions(contentId: string): Promise<QuizQuestion[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<QuizQuestion[]>(`/quiz/${contentId}`);

  const document = bundle.anyById(contentId) as unknown as { quiz?: QuizQuestion[] } | undefined;
  return (document?.quiz ?? []).map(({ answerIndex: _answer, explanation: _explanation, ...rest }) => rest);
}

export async function evaluateQuiz(contentId: string, answers: Record<string, number>): Promise<QuizResult> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    return request<QuizResult>(`/quiz/${contentId}/evaluate`, {
      method: 'POST',
...
export async function practiceQuestions(count: number): Promise<PracticeQuestion[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<PracticeQuestion[]>(`/practice/pattern-recognition?count=${count}`);
...
export interface PracticeResult {
  ...
  total: number;
  percent: number;
  results: { id: string; given: string; answer: string; correct: boolean; signals: string[]; explanation: string }[];
}

export async function evaluatePractice(answers: Record<string, string>): Promise<PracticeResult> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    return request<PracticeResult>('/practice/pattern-recognition/evaluate', {
      method: 'POST',
      body: JSON.stringify({ answers }),
    });
  }
...
export async function search(query: string, limit = 20): Promise<SearchHit[]> {
  ...
  if (mode === 'online') {
    return request<SearchHit[]>(`/search?q=${encodeURIComponent(query)}&limit=${limit}`);
  }
  ...
  const documents: { summary: ContentSummary; haystack: string; title: string }[] = [];
  const add = (items: Iterable<Lesson | Problem | Concept | CaseStudy>, type: ContentSummary['type']) => {
  ...
  return documents
    .map((document) => {
      ...
      for (const token of tokens) {
        if (document.title.includes(token)) score += 20;
        else if ((document.summary.group ?? '').toLowerCase().includes(token)) score += 8;
        else if ((document.summary.summary ?? '').toLowerCase().includes(token)) score += 6;
        else if (document.haystack.includes(token)) score += 1;
        else return null;            // AND semantics, matching the backend
      }
      return { ...document.summary, score };
    })
    .filter((hit): hit is ContentSummary & { score: number } => hit !== null)
...
export async function notes(): Promise<NoteRow[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<NoteRow[]>('/notes');
  const store = local.read();
...
export async function note(contentId: string): Promise<string> {
  if (mode === 'online') {
    const row = await request<{ body: string }>(`/notes/${contentId}`);
    return row.body;
  }
  return local.read().notes[contentId]?.body ?? '';
}

export async function saveNote(contentId: string, body: string): Promise<void> {
  if (mode === 'online') {
    await request<void>(`/notes/${contentId}`, { method: 'PUT', body: JSON.stringify({ body }) });
    return;
  }
  local.saveNote(contentId, body);
}

export async function bookmarks(): Promise<NoteRow[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<NoteRow[]>('/bookmarks');
  const store = local.read();
...
export async function toggleBookmark(contentId: string, on: boolean): Promise<void> {
  if (mode === 'online') {
    await request<void>(`/bookmarks/${contentId}`, { method: on ? 'POST' : 'DELETE' });
    return;
  }
...
export async function importProgress(text: string): Promise<void> {
  if (mode !== 'online') await ensureBundle();
  ...
  if (mode === 'online') {
    await request<void>('/progress/import', { method: 'POST', body: text });
    return;
  }

  const lessonsIn = (payload.lessons ?? []) as { contentId: string; status: ProgressStatus; percent: number; contentType?: string; updatedAt?: string }[];
  const attemptsIn = (payload.attempts ?? []) as { problem_id: string; pattern_id?: string; solved: number; confidence?: number; attempted_at: string }[];
  const notesIn = (payload.notes ?? []) as { contentId: string; body: string; updatedAt?: string }[];
  const bookmarksIn = (payload.bookmarks ?? []) as { contentId: string; createdAt?: string }[];
  ...
  local.replace({
    ...
    quizzes: Object.fromEntries(
      ((payload.quizzes ?? []) as { contentId: string; bestScore: number; total: number; attempts: number; lastTaken: string }[]).map(
        ({ contentId, ...rest }) => [contentId, rest],
      ),
    ),
    flags: (payload.flags ?? {}) as Record<string, { difficult: boolean; mastered: boolean }>,
    notes: Object.fromEntries(
      notesIn.map((row) => [row.contentId, { body: row.body, updatedAt: row.updatedAt ?? new Date().toISOString() }]),
    ),
    bookmarks: Object.fromEntries(bookmarksIn.map((row) => [row.contentId, row.createdAt ?? new Date().toISOString()])),
    activeDays: (payload.activeDays ?? []) as string[],
  });
}

// ------------------------------------------------------------- interview prep

const LEVEL_ORDER: InterviewLevel[] = ['junior', 'mid', 'senior'];

function countLevels(questions: { level: InterviewLevel }[]): Record<InterviewLevel, number> {
  const counts: Record<InterviewLevel, number> = { junior: 0, mid: 0, senior: 0 };
  for (const question of questions) counts[question.level] = (counts[question.level] ?? 0) + 1;
  return counts;
}

/** Every track with live question counts — the interview index page. */
export async function interviewTracks(): Promise<InterviewTrack[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<InterviewTrack[]>('/interview/tracks');
...
export async function interviewTrack(id: string): Promise<InterviewTrack & { topics: InterviewTopic[] }> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<InterviewTrack & { topics: InterviewTopic[] }>(`/interview/tracks/${id}`);

  const track = bundle.interviewTracks.find((row) => row.id === id);
  if (!track) throw new Error(`No interview track with id "${id}"`);

  const declared = (track.topics as string[]) ?? [];
  const topics: InterviewTopic[] = [...bundle.questionSets.values()]
...
export async function questionSet(id: string): Promise<QuestionSet> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<QuestionSet>(`/interview/sets/${id}`);

  const set = bundle.questionSets.get(id);
  if (!set) throw new Error(`No question set with id "${id}"`);
...
export async function interviewQuestions(filters: QuestionFilters = {}): Promise<InterviewQuestion[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(filters)) if (value) params.set(key, value);
    return request<InterviewQuestion[]>(`/interview/questions?${params.toString()}`);
  }

  const needle = filters.q?.toLowerCase().trim();
  return [...bundle.interviewQuestions.values()]
    .filter((question) => {
      if (filters.track && question.trackId !== filters.track) return false;
...
export async function interviewQuestion(id: string): Promise<InterviewQuestion> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<InterviewQuestion>(`/interview/questions/${id}`);

  const question = bundle.interviewQuestions.get(id);
  if (!question) throw new Error(`No interview question with id "${id}"`);
  return question;
}
 ...
 */
export async function interviewDrill(
  options: { track?: string; level?: InterviewLevel; count?: number; seed?: number } = {},
  ...
  if (mode === 'online') {
    ...
    return request<InterviewQuestion[]>(`/interview/drill?${params.toString()}`);
  }
 ...
 */
export async function typeOf(id: string): Promise<string> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    try {
      const body = await request<{ type?: string }>(`/meta/type/${id}`);
...
export async function languages(): Promise<LanguageSummary[]> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') return request<LanguageSummary[]>('/i18n/languages');
...
export async function uiStrings(code: string): Promise<Record<string, string>> {
  if (mode !== 'online') await ensureBundle();
  if (mode === 'online') {
    try {
      const body = await request<{ strings?: Record<string, string> }>(`/i18n/strings/${code}`);
      return body.strings ?? {};
    } catch {
      return {};
    }
