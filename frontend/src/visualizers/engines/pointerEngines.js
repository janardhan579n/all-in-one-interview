...
export const twoPointersOpposite = (input) => {
    ...
    const scene = (l, r, extra) => {
        ...
        for (let i = r + 1; i < values.length; i++)
            cellStates[i] = 'reject';
        if (l < values.length)
            cellStates[l] = extra?.matched ? 'match' : 'active';
        if (r >= 0)
            cellStates[r] = extra?.matched ? 'match' : 'active';
        if (extra?.dropped !== undefined)
            cellStates[extra.dropped] = 'reject';
        return {
            array: { values, states: cellStates, caption: `sorted nums (target ${target})` },
            pointers: [
                { name: 'left', index: Math.max(0, Math.min(l, values.length - 1)), tone: 'brand' },
                { name: 'right', index: Math.max(0, r), tone: 'warn' },
            ],
        };
    };
    steps.push({
...
export const twoPointersPalindrome = (input) => {
    const source = text(input, 'text', 'racecar');
    const characters = source.split('');
    const steps = [];
    let left = 0;
    let right = characters.length - 1;
    const scene = (l, r, state) => {
        const cellStates = states(characters.length);
        for (let i = 0; i < l; i++)
            cellStates[i] = 'visited';
        for (let i = r + 1; i < characters.length; i++)
            cellStates[i] = 'visited';
        ...
        return {
            ...
            pointers: [
                { name: 'left', index: Math.max(0, Math.min(l, characters.length - 1)), tone: 'brand' },
                { name: 'right', index: Math.max(0, r), tone: 'warn' },
            ],
        };
    };
 ...
 */
export const twoPointersSameDirection = (input) => {
    const values = [...numbers(input, 'array', [1, 1, 2, 2, 2, 3, 4, 4])];
    const steps = [];
    let slow = 0;
    const scene = (s, f, wrote) => {
        const cellStates = states(values.length);
        for (let i = 0; i <= s; i++)
            cellStates[i] = 'match';
        for (let i = s + 1; i < f; i++)
        ...
        if (f < values.length)
            cellStates[f] = 'active';
        if (wrote)
            cellStates[s] = 'inWindow';
        return {
            array: { values: [...values], states: cellStates, caption: 'nums (kept region highlighted)' },
            pointers: [
                { name: 'slow', index: s, tone: 'good' },
                { name: 'fast', index: Math.min(f, values.length - 1), tone: 'brand' },
            ],
        };
    };
    steps.push({
