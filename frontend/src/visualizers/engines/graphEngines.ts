import { cap, text, type CellState, type Engine, type GraphPanel, type Step } from '../types';

type EdgeInput = [string, string] | { from: string; to: string; label?: string };

function readNodes(input: Record<string, unknown>): string[] {
  const raw = input.nodes;
  if (Array.isArray(raw) && raw.every((n) => typeof n === 'string')) return raw as string[];
  return ['A', 'B', 'C', 'D', 'E', 'F'];
}

function readEdges(input: Record<string, unknown>): { from: string; to: string; label?: string }[] {
  const raw = input.edges;
  if (!Array.isArray(raw)) return [];
  return (raw as EdgeInput[]).map((edge) =>
 ...
 */
function layout(nodes: string[]): Record<string, { x: number; y: number }> {
...
function adjacency(nodes: string[], edges: { from: string; to: string }[], directed: boolean) {
  const map = new Map<string, string[]>(nodes.map((id) => [id, []]));
  edges.forEach(({ from, to }) => {
    map.get(from)?.push(to);
    if (!directed) map.get(to)?.push(from);
  });
  return map;
}
...
function graphPanel(
  ...
  marks: Map<string, CellState>,
  edgeMarks: Map<string, 'idle' | 'active' | 'tree' | 'back'>,
  directed: boolean,
  caption: string,
): GraphPanel {
  return {
...
export const graphRepresentation: Engine = (input) => {
  const nodes = readNodes(input);
  const edges = readEdges(input);
  const directed = input.directed === true;
  const positions = layout(nodes);
  const steps: Step[] = [];
  const marks = new Map<string, CellState>();
  const edgeMarks = new Map<string, 'idle' | 'active' | 'tree' | 'back'>();

  const adjacencyRows: (string | number)[][] = nodes.map((id) => [id, '']);
  ...
  edges.forEach((edge, index) => {
    ...
    if (!directed) matrix[toIndex][fromIndex] = 1;

    const appendTo = (nodeIndex: number, neighbour: string) => {
    ...
    if (!directed) appendTo(toIndex, edge.from);
...
export const connectedComponents: Engine = (input) => {
  const nodes = readNodes(input);
  const edges = readEdges(input);
  const positions = layout(nodes);
 ...
 * and DFS does not.
 */
export const graphBfs: Engine = (input) => {
  const nodes = readNodes(input);
  const edges = readEdges(input);
    ...
    for (let i = 0; i < levelSize; i++) {
      ...
      for (const next of adjacent.get(node) ?? []) {
        if (!visited.has(next)) {
          ...
          nextQueue.push(next);
          edgeMarks.set(`${node}->${next}`, 'tree');
          if (marks.get(next) !== 'target') marks.set(next, 'inWindow');
          steps.push({
            scene: {
              graph: graphPanel(nodes, edges, positions, marks, edgeMarks, input.directed === true, `distance ${distance}`),
              queue: { items: [...queue.slice(i + 1), ...nextQueue], caption: 'queue' },
...
export const graphDfs: Engine = (input) => {
  const nodes = readNodes(input);
  const edges = readEdges(input);
  const start = text(input, 'start', nodes[0]);
  const positions = layout(nodes);
  ...
  const visit = (node: string, from?: string) => {
    ...
    if (from) edgeMarks.set(`${from}->${node}`, 'tree');
 ...
 */
export const directedCycle: Engine = (input) => {
  const nodes = readNodes(input);
  const edges = readEdges(input);
  const start = text(input, 'start', nodes[0]);
  const positions = layout(nodes);
  ...
  const marksNow = () => {
    const marks = new Map<string, CellState>();
    nodes.forEach((id) => {
      if (onPath.has(id)) marks.set(id, 'active');
      else if (visited.has(id)) marks.set(id, 'visited');
    });
    return marks;
  };
  ...
  const visit = (node: string, from?: string): boolean => {
    if (onPath.has(node)) {
      if (from) edgeMarks.set(`${from}->${node}`, 'back');
      cycleFound = true;
 ...
 */
export const gridBfs: Engine = (input) => {
  const grid = (Array.isArray(input.grid) ? input.grid : [[0]]) as number[][];
  const diagonal = input.diagonal === true;
  const rows = grid.length;
  const cols = grid[0].length;

  const declaredSources = Array.isArray(input.sources) ? (input.sources as number[][]) : null;
  const start = (Array.isArray(input.start) ? input.start : [0, 0]) as number[];
  const sources: number[][] = declaredSources?.length ? declaredSources : [[start[0], start[1]]];
  const target = Array.isArray(input.target) ? (input.target as number[]) : null;
  const multiSource = sources.length > 1 || target === null;

  const steps: Step[] = [];
  const cellStates: CellState[][] = grid.map((row) => row.map((cell) => (cell === 1 ? 'reject' : 'idle')));
  ...
  if (target) cellStates[target[0]][target[1]] = 'target';
  let distance = 0;
  ...
  snapshot(
    multiSource
      ...
      : `Each open cell is a node; its neighbours are the ${DIRS.length} adjacent cells. You never build this graph — you generate neighbours on demand.`,
    2,
    { distance, sources: sources.length },
  );
  ...
  while (queue.length > 0) {
    ...
    for (let i = 0; i < levelSize; i++) {
      const [r, c] = queue[i];
      if (cellStates[r][c] !== 'target') cellStates[r][c] = 'visited';

      if (target && r === target[0] && c === target[1]) {
        cellStates[r][c] = 'match';
        snapshot(
          `Reached the exit in ${distance} steps. Every cell was enqueued at most once, so this is O(rows × cols) — not O(paths).`,
          4,
          { steps: distance },
        );
        steps[steps.length - 1] = { ...steps[steps.length - 1], done: true };
      ...
      for (const [dr, dc] of DIRS) {
        const nr = r + dr;
        const nc = c + dc;
        if (nr < 0 || nr >= rows || nc < 0 || nc >= cols) continue;   // bounds BEFORE indexing
        if (seen[nr][nc] || grid[nr][nc] === 1) continue;
        seen[nr][nc] = true;
        reached++;
        if (cellStates[nr][nc] !== 'target') cellStates[nr][nc] = 'inWindow';
    ...
    snapshot(
      multiSource
        ...
        : `Ring ${distance}: ${queue.length} cell${queue.length === 1 ? '' : 's'} are exactly ${distance} steps away.`,
      10,
      { distance, frontier: queue.length },
    );
  }
  ...
  if (multiSource) {
    snapshot(
      reached === open
        ...
        : `The frontier is exhausted after ${distance} round${distance === 1 ? '' : 's'}, but ${open - reached} open cell${open - reached === 1 ? ' was' : 's were'} never reached — walled off from every source. That is the case the problem asks you to report as impossible.`,
      12,
      { rounds: distance, reached, unreachable: open - reached },
    );
  } else {
...
export const gridDfs: Engine = (input) => {
  const grid = structuredClone((Array.isArray(input.grid) ? input.grid : [[0]]) as number[][]);
  const steps: Step[] = [];
  const rows = grid.length;
  const cols = grid[0].length;
  const cellStates: CellState[][] = grid.map((row) => row.map((cell) => (cell === 1 ? 'idle' : 'reject')));
  let islands = 0;
