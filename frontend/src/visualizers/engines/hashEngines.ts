import { cap, num, numbers, states, type CellState, type CellValue, type Engine, type Step } from '../types';

/** Prefix sums: build once, then answer any range with a single subtraction. */
export const prefixSum: Engine = (input) => {
  const values = numbers(input, 'array', [3, 1, 4, 1, 5, 9, 2, 6]);
  const queries = (Array.isArray(input.queries) ? input.queries : [[1, 3]]) as number[][];
  const steps: Step[] = [];
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    cellStates[i] = 'active';
    const prefixStates: CellState[] = states(prefix.length, 'visited');
  ...
  for (const [from, to] of queries) {
    ...
    const cellStates: CellState[] = states(values.length);
    for (let i = from; i <= to; i++) cellStates[i] = 'inWindow';
    const prefixStates: CellState[] = states(prefix.length);
 ...
 * start at index 0.
 */
export const prefixSumHash: Engine = (input) => {
  ...
  const bucketsOf = (highlightKey?: number) => ({
    ...
    buckets: [
      {
        ...
        entries: [...seen.entries()].map(([key, value]) => ({
          key: String(key),
          value,
          state: (key === highlightKey ? 'match' : 'idle') as CellState,
        })),
      },
    ],
  });
  ...
  for (let i = 0; i < values.length; i++) {
    running += values[i];
    const cellStates: CellState[] = states(values.length);
    for (let j = 0; j < i; j++) cellStates[j] = 'visited';
    cellStates[i] = 'active';
...
export const hashingOps: Engine = (input) => {
  ...
  const buckets: { key: string; value: CellValue; state?: CellState }[][] =
...
export const twoSumHash: Engine = (input) => {
    ...
    buckets: [
      {
        ...
        entries: [...seen.entries()].map(([key, value]) => ({
          key: String(key),
          value,
          state: (key === highlight ? 'match' : 'idle') as CellState,
        })),
      },
    ],
  });
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (seen.has(needed)) {
      const partner = seen.get(needed) as number;
      const matched: CellState[] = states(values.length, 'visited');
      matched[i] = 'match';
