import { cap, numbers, text, type CellState, type Engine, type ListNodeView, type Step } from '../types';
...
function toNodes(values: ListState[], marks: Record<string, CellState>): ListNodeView[] {
...
export const linkedListOps: Engine = (input) => {
  const initial = numbers(input, 'values', [10, 20, 30, 40]);
  const operations = (Array.isArray(input.operations) ? input.operations : []) as {
    op: 'traverse' | 'insertAfter' | 'deleteAt' | 'insertHead';
    index?: number;
    value?: number;
  }[];

  const steps: Step[] = [];
  let nodes: ListState[] = initial.map((value, i) => ({ id: `n${i}`, value }));
  let nextId = initial.length;

  const scene = (marks: Record<string, CellState>, caption: string, pointers: { name: string; nodeId: string | null }[] = []) => ({
    list: {
      nodes: toNodes(nodes, marks),
      pointers: pointers.map((p) => ({ ...p, tone: 'brand' as const })),
      caption,
    },
  });
  ...
  for (const operation of operations) {
    if (operation.op === 'traverse') {
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (let k = 0; k < i; k++) marks[nodes[k].id] = 'visited';
    ...
    if (operation.op === 'insertAfter' && operation.index !== undefined && operation.value !== undefined) {
      ...
      const fresh: ListState = { id: `n${nextId++}`, value: operation.value };
 ...
 */
export const linkedListReverse: Engine = (input) => {
  const values = numbers(input, 'values', [1, 2, 3, 4, 5]);
  const steps: Step[] = [];

  const nodes: ListState[] = values.map((value, i) => ({ id: `n${i}`, value }));
  let order = [...nodes];
  let reversed: ListState[] = [];

  const scene = (caption: string, marks: Record<string, CellState>, pointers: { name: string; nodeId: string | null }[]) => ({
    list: {
  ...
  while (order.length > 0) {
    ...
    steps.push({
      scene: scene('remember the rest', { [current.id]: 'active', ...(next ? { [next.id]: 'target' } : {}) },
        [{ name: 'curr', nodeId: current.id }, { name: 'next', nodeId: next?.id ?? null }]),
      codeLine: 4,
      explain: next
    ...
    steps.push({
      scene: scene('link flipped', { [current.id]: 'match' },
        [{ name: 'prev', nodeId: current.id }, { name: 'curr', nodeId: next?.id ?? null }]),
      codeLine: 5,
 ...
 */
export const fastSlowPointer: Engine = (input) => {
  const values = numbers(input, 'values', [1, 2, 3, 4, 5, 6, 7]);
  const cycleAt = typeof input.cycleAt === 'number' ? (input.cycleAt as number) : null;
  const mode = text(input, 'mode', 'middle') as 'middle' | 'cycle' | 'entry';
  const steps: Step[] = [];

  const nodes: ListState[] = values.map((value, i) => ({ id: `n${i}`, value }));
  const nextIndex = (i: number): number | null => {
    if (i + 1 < nodes.length) return i + 1;
    return cycleAt !== null ? cycleAt : null;
  };

  const scene = (slow: number | null, fast: number | null, caption: string, extra: Record<string, CellState> = {}) => {
    const marks: Record<string, CellState> = { ...extra };
    if (slow !== null) marks[nodes[slow].id] = marks[nodes[slow].id] ?? 'active';
    if (fast !== null) marks[nodes[fast].id] = fast === slow ? 'match' : 'target';
    return {
      list: {
        nodes: nodes.map((node) => ({ id: node.id, value: node.value, state: marks[node.id] ?? 'idle' })),
        pointers: [
          { name: 'slow', nodeId: slow !== null ? nodes[slow].id : null, tone: 'good' as const },
          { name: 'fast', nodeId: fast !== null ? nodes[fast].id : null, tone: 'warn' as const },
        ],
        cycleTo: cycleAt,
        caption,
      },
  ...
  while (fast !== null) {
    ...
    if (slow !== null && slow === fast) {
      ...
      steps.push({
        ...
        codeLine: 3,
        explain:
          'Because a + b = mc, the distance from the head to the entry equals the distance from the meeting point to the entry. So walk both at the SAME speed.',
        vars: { walker: values[walker], chaser: values[chaser] },
      });

      while (walker !== chaser) {
        walker = nextIndex(walker) as number;
        chaser = nextIndex(chaser) as number;
        steps.push({
...
function distanceInCycle(slow: number, fast: number, length: number, cycleAt: number): number {
  const cycleLength = length - cycleAt;
  const positionOf = (index: number) => (index < cycleAt ? -1 : index - cycleAt);
  const s = positionOf(slow);
  const f = positionOf(fast);
  if (s < 0 || f < 0) return Math.abs(fast - slow);
