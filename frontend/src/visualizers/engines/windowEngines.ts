 ...
 */
export const slidingWindowFixed: Engine = (input) => {
...
function windowScene(
  ...
  if (windowReal) {
    for (let i = Math.max(0, left); i <= right && i < values.length; i++) cellStates[i] = 'inWindow';
  }
  if (touched >= 0) cellStates[touched] = 'active';
  for (let i = bestRange[0]; i <= bestRange[1]; i++) {
    if (cellStates[i] === 'idle') cellStates[i] = 'visited';
  }

  return {
    array: { values, states: cellStates, caption: 'nums' },
    pointers: [
      { name: 'left', index: Math.max(0, left), tone: 'warn' as const },
      { name: 'right', index: Math.max(0, right), tone: 'brand' as const },
    ],
    ranges: windowReal
      ? [{ name: 'window', from: Math.max(0, left), to: Math.max(0, right), tone: 'brand' as const }]
      : [],
  };
}
 ...
 */
export const slidingWindowVariable: Engine = (input) => {
  ...
  function scene(
    ...
    windowRight: number,
    touched: number,
    bestSoFar: [number, number],
    seen: Map<string, number>,
  ) {
    const cellStates: CellState[] = states(chars.length);
    for (let i = windowLeft; i <= windowRight && i < chars.length; i++) cellStates[i] = 'inWindow';
    if (touched >= 0 && touched < chars.length) cellStates[touched] = 'active';
    ...
    return {
      ...
      pointers: [
        { name: 'left', index: Math.max(0, windowLeft), tone: 'warn' as const },
        { name: 'right', index: Math.max(0, windowRight), tone: 'brand' as const },
      ],
      buckets: {
        caption: 'lastSeen',
        buckets: [
          {
            ...
            entries: [...seen.entries()].map(([key, value]) => ({
              key,
              value,
              state: (value >= windowLeft ? 'inWindow' : 'idle') as CellState,
            })),
          },
        ],
      },
 ...
 */
export const slidingWindowShrink: Engine = (input) => {
  ...
  function build(nums: number[], windowLeft: number, windowRight: number, touched: number, bestSoFar: [number, number]) {
    const cellStates: CellState[] = states(nums.length);
    for (let i = windowLeft; i <= windowRight && i < nums.length; i++) cellStates[i] = 'inWindow';
    if (touched >= 0 && touched < nums.length) cellStates[touched] = 'reject';
    for (let i = bestSoFar[0]; i <= bestSoFar[1]; i++) {
      if (cellStates[i] === 'idle') cellStates[i] = 'visited';
    }
    return {
      ...
      ranges:
        windowRight >= windowLeft
          ? [{ name: 'window', from: windowLeft, to: windowRight, tone: 'brand' as const }]
          : [],
    };
  }
};
