...
function searchScene(values: number[], low: number, high: number, mid: number, target: number, found: boolean) {
  const cellStates: CellState[] = states(values.length);
  for (let i = 0; i < values.length; i++) {
    if (i < low || i > high) cellStates[i] = 'reject';
    else cellStates[i] = 'inWindow';
  }
  if (mid >= 0 && mid < values.length) cellStates[mid] = found ? 'match' : 'active';

  return {
    array: { values, states: cellStates, caption: `sorted nums (target ${target})` },
    pointers: [
      { name: 'low', index: Math.max(0, Math.min(low, values.length - 1)), tone: 'good' as const },
      { name: 'mid', index: Math.max(0, Math.min(mid, values.length - 1)), tone: 'brand' as const },
      { name: 'high', index: Math.max(0, Math.min(high, values.length - 1)), tone: 'warn' as const },
    ],
    ranges: low <= high ? [{ name: 'candidates', from: low, to: high, tone: 'info' as const }] : [],
  };
}

/** Classic exact-match binary search. Watch the candidate range halve every probe. */
export const binarySearch: Engine = (input) => {
 ...
 */
export const binarySearchBoundary: Engine = (input) => {
 ...
 */
export const binarySearchAnswer: Engine = (input) => {
