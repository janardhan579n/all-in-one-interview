import { cap, numbers, states, type CellState, type Engine, type Step } from '../types';
...
function heapLabels(size: number): string[] {
...
export const heapOps: Engine = (input) => {
  const operations = (Array.isArray(input.operations) ? input.operations : []) as HeapOperation[];
  const resolved: HeapOperation[] = operations.length
    ? operations
    : [
        { op: 'insert', value: 5 },
        { op: 'insert', value: 3 },
  ...
  const snapshot = (
    ...
    for (const [index, state] of Object.entries(marked)) {
      ...
      if (position >= 0 && position < heap.length) cellStates[position] = state;
    }
    steps.push({
      scene: {
        array: {
  ...
  for (const operation of resolved) {
    if (operation.op === 'insert' && operation.value !== undefined) {
      ...
      snapshot(
        `insert(${operation.value}) — always append at the end first. That keeps the tree complete.`,
        2,
        { [current]: 'active' },
        { inserted: operation.value },
      );
      ...
      while (current > 0) {
        ...
        if (heap[parent] <= heap[current]) {
          snapshot(
            `${heap[current]} is not smaller than its parent ${heap[parent]} — the heap property holds, stop.`,
            4,
            { [current]: 'match', [parent]: 'visited' },
            { child: current, parent },
          );
          break;
        }
        snapshot(
          `${heap[current]} < parent ${heap[parent]} — swap them and keep climbing.`,
          5,
          { [current]: 'active', [parent]: 'target' },
          { child: current, parent },
        );
    ...
    if (operation.op === 'extract') {
      ...
      snapshot(
        `extractMin() → ${minimum}. The root is always the smallest; that is the whole guarantee a heap gives.`,
        8,
        { 0: 'match' },
        { min: minimum },
      );
      ...
      if (heap.length > 0) {
        heap[0] = last;
        snapshot(
          `Move the last element (${last}) to the root, so the tree stays complete. Now it is almost certainly in the wrong place.`,
          9,
          { 0: 'active' },
        );
        ...
        for (;;) {
          ...
          if (smallest === current) {
            snapshot(
              `${heap[current]} is smaller than both children — the heap property is restored.`,
              12,
              { [current]: 'match' },
              { at: current },
            );
          ...
          snapshot(
            `Swap with the smaller child ${heap[smallest]}. Always the smaller one — swapping with the larger would break the other subtree.`,
            11,
            { [current]: 'active', [smallest]: 'target' },
            { at: current, swapWith: smallest },
          );
 ...
 */
export const topKHeap: Engine = (input) => {
  const values = numbers(input, 'array', [4, 1, 7, 3, 9, 2, 8]);
  const k = Math.max(1, Math.min(numbers(input, 'array', values).length, (input.k as number) ?? 3));
  const heap: number[] = [];
  const steps: Step[] = [];

  const snapshot = (explain: string, codeLine: number, index: number, heapState: CellState = 'inWindow') => {
    const cellStates = states(values.length);
    for (let i = 0; i < index; i++) cellStates[i] = 'visited';
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    if (value > heap[0]) {
      ...
      siftDown();
      snapshot(
        `${value} beats the weakest survivor ${evicted} — evict ${evicted}, keep ${value}.`,
        5,
        index,
      );
    } else {
      snapshot(
        `${value} is not larger than the weakest survivor ${heap[0]} — it cannot be in the top ${k}. Discard it.`,
        7,
        index,
        'reject',
      );
