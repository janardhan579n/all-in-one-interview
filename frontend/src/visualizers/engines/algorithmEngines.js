...
function timeline(intervals, merged, activeIndex, span) {
    const nodes = [];
...
export const mergeIntervals = (input) => {
    ...
    steps.push({
        ...
        codeLine: 1,
        explain: 'Merge every pair of intervals that overlap. Unsorted, "which ones overlap" is a question about every pair.',
    ...
    steps.push({
        ...
        codeLine: 2,
        explain: 'Sort by start. This is the whole trick: once sorted, an interval can only ever overlap the one currently being built — never anything earlier, because earlier intervals start earlier and have already been closed off.',
        vars: { sortedBy: 'start' },
    });
    const merged = [];
...
export const mergeSortSteps = (input) => {
    ...
    let comparisons = 0;
    const snapshot = (explain, codeLine, from, to, depth) => {
        const cellStates = states(working.length);
        for (let i = from; i <= to && i < working.length; i++)
    ...
    const merge = (from, middle, to, depth) => {
        ...
        while (l < left.length)
            working[write++] = left[l++];
        while (r < right.length)
            working[write++] = right[r++];
        snapshot(`Merge the two sorted halves into [${working.slice(from, to + 1).join(', ')}]. Merging two sorted runs is one pass — that is the only reason splitting was worth it.`, 8, from, to, depth);
    };
    const sort = (from, to, depth) => {
        if (from >= to)
            return;
...
export const quickSortSteps = (input) => {
    ...
    const steps = [];
    let swaps = 0;
    const snapshot = (explain, codeLine, from, to, pivotIndex, cursor) => {
        const cellStates = states(working.length);
        for (let i = from; i <= to && i < working.length; i++)
            cellStates[i] = 'inWindow';
        if (cursor >= 0 && cursor < working.length)
            cellStates[cursor] = 'active';
        if (pivotIndex >= 0 && pivotIndex < working.length)
            cellStates[pivotIndex] = 'target';
        steps.push({
    ...
    const partition = (from, to) => {
        ...
        swaps += 1;
        snapshot(`Put the pivot between the two sides. ${pivot} is now in its final position — that is the guarantee partitioning gives, and it is why no merge step is needed.`, 8, from, to, boundary, -1);
        return boundary;
    };
    const sort = (from, to) => {
        if (from >= to)
...
export const bitOps = (input) => {
    const value = num(input, 'value', 12);
    const steps = [];
    const show = (label, result, explain, codeLine, highlight = () => 'idle') => {
        steps.push({
            scene: {
                array: {
                    values: bits(value),
    ...
    show(`value & 1`, value & 1, `AND with 1 keeps only the last bit, which is 0 for even and 1 for odd. Faster to read than % 2 once you see it, and identical in cost.`, 4, (index) => (index === BIT_WIDTH - 1 ? 'active' : 'idle'));
    show(`value & (value - 1)`, value & (value - 1), `Subtracting 1 flips the lowest set bit and everything below it; ANDing with the original therefore clears exactly that one bit. Loop this and the number of iterations is the number of set bits — Brian Kernighan's trick.`, 5);
    show(`value & -value`, value & -value, `Negation in two's complement is "flip all bits and add 1", which leaves the lowest set bit as the only bit agreeing with the original. So this isolates it — the operation a Fenwick tree is built on.`, 6);
...
export const topologicalSort = (input) => {
    ...
    const order = [];
    const steps = [];
    const snapshot = (explain, codeLine, active, queue) => {
        steps.push({
            scene: {
                graph: {
                    directed: true,
    ...
    snapshot('Count how many prerequisites each node has. Anything with a count of zero can go first — there is nothing to wait for.', 2, null, queue);
    ...
    snapshot(cyclic
        ...
        : `Order: ${order.join(' → ')}. Every edge points forwards in this list, which is exactly what a topological order means.`, 11, null, []);
    steps[steps.length - 1].done = true;
    return cap(steps);
};
export const dijkstra = (input) => {
    const graph = input.graph?.nodes?.length
        ? input.graph
        : {
            nodes: [
                { id: 'a', x: 70, y: 100 },
    ...
    const source = input.source ?? graph.nodes[0].id;
    const distance = new Map(graph.nodes.map((node) => [node.id, Infinity]));
    ...
    const snapshot = (explain, codeLine, active, relaxing) => {
        steps.push({
            scene: {
                graph: {
                    nodes: graph.nodes.map((node) => ({
                    ...
                    edges: graph.edges.map((edge) => ({
                        ...
                        state: relaxing && relaxing.from === edge.from && relaxing.to === edge.to
                            ? 'active'
    ...
    for (;;) {
        ...
        if (current === null)
            break;
        settled.add(current);
        snapshot(`Settle ${current} at distance ${best}. It is the nearest unsettled node, and because no edge is negative, nothing discovered later can beat it — that single fact is what makes the greedy choice safe.`, 5, current);
        for (const edge of graph.edges.filter((e) => e.from === current)) {
            const candidate = best + edge.weight;
            const known = distance.get(edge.to) ?? Infinity;
            if (candidate < known) {
                distance.set(edge.to, candidate);
                snapshot(`Going via ${current} reaches ${edge.to} in ${candidate}, better than ${known === Infinity ? '∞' : known}. Update it.`, 8, current, { from: edge.from, to: edge.to });
            }
            else {
                snapshot(`Via ${current}, ${edge.to} would cost ${candidate} — no better than the ${known} we already have. Leave it.`, 7, current, { from: edge.from, to: edge.to });
            }
        }
    }
    snapshot(`Done. Every node holds its shortest distance from ${source}. With a negative edge this would be wrong — a settled node could still be improved later, which is exactly the case Bellman-Ford handles and Dijkstra does not.`, 11, null);
    steps[steps.length - 1].done = true;
    return cap(steps);
};
/* -------------------------------------------------------------- matrix spiral */
export const matrixSpiral = (input) => {
    ...
    const order = [];
    const steps = [];
    const snapshot = (explain, codeLine, at, vars) => {
        steps.push({
            scene: {
                grid: {
                    cells: matrix,
                    states: visited.map((row, r) => row.map((seen, c) => (at && at[0] === r && at[1] === c ? 'active' : seen ? 'visited' : 'idle'))),
