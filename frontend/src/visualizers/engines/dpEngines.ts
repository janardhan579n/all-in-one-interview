import { cap, num, text, type CellState, type CellValue, type Engine, type Step } from '../types';
 ...
 */
export const dp1d: Engine = (input) => {
  const kind = text(input, 'kind', 'climbStairs');
  const n = Math.min(num(input, 'n', 8), 20);
  const steps: Step[] = [];

  const dp: CellValue[] = Array.from({ length: n + 1 }, () => '?');
  const cellStates: CellState[] = Array.from({ length: n + 1 }, () => 'idle');

  const snapshot = (explain: string, codeLine: number, vars?: Record<string, CellValue>) => {
    steps.push({
      scene: {
        array: {
          values: [...dp],
  ...
  snapshot(
    `State definition first, in words: dp[i] = the number of distinct ways to reach step i. If you cannot write that sentence, no amount of index fiddling will save the solution. (${kind})`,
    1,
  );
  ...
  cellStates[n] = 'match';
  snapshot(
    `Answer: ${dp[n]}. The naive recursion would have made about 2^${n} calls to compute this; the table computes each of the ${n + 1} states exactly once. And since every cell reads only the previous two, the whole array can be replaced by two variables — O(1) space.`,
    7,
    { answer: dp[n] },
  );
 ...
 */
export const dpGrid: Engine = (input) => {
  const rows = Math.min(num(input, 'rows', 4), 8);
  const cols = Math.min(num(input, 'cols', 5), 8);
  const steps: Step[] = [];

  const dp: CellValue[][] = Array.from({ length: rows }, () => Array.from({ length: cols }, () => '' as CellValue));
  const cellStates: CellState[][] = Array.from({ length: rows }, () => Array.from({ length: cols }, () => 'idle' as CellState));
  ...
  snapshot(
    `The robot moves only right or down. State: dp[r][c] = how many distinct paths reach cell (r, c). Naive recursion is O(2^(rows+cols)); this table is O(rows × cols).`,
    1,
  );
    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let rr = 0; rr < rows; rr++) {
        for (let cc = 0; cc < cols; cc++) {
          if (dp[rr][cc] !== '') cellStates[rr][cc] = 'visited';
        }
      }
      ...
      if (r === 0 || c === 0) {
        ...
        snapshot(
          r === 0 && c === 0
            ...
            : `Edge cell (${r}, ${c}): only one route reaches it — straight along the ${r === 0 ? 'top row' : 'left column'}.`,
          4,
          { [`dp[${r}][${c}]`]: 1 },
        );
      } else {
  ...
  snapshot(
    `${dp[rows - 1][cols - 1]} distinct paths across a ${rows}×${cols} grid. Each row depends only on the row above, so a single rolling array of ${cols} values is enough — O(n) space instead of O(m·n).`,
    8,
