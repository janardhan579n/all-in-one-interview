import { cap, num, numbers, text } from '../types';
/**
 ...
 * recomputed.
 */
export const recursionTrace = (input) => {
    const kind = text(input, 'kind', 'factorial');
            ...
            scene: {
                ...
                table: {
                    ...
                    rows: [...callCount.entries()]
                        .sort((a, b) => a[0] - b[0])
                        .map(([key, count]) => [key, count, computed.has(key) ? computed.get(key) : '…']),
                    cursor: null,
    ...
    const fib = (value) => {
        ...
        snapshot(callCount.get(value) > 1
            ? `Call fib(${value}) — for the ${callCount.get(value)}${ordinal(callCount.get(value))} time. The work is being repeated.`
            : `Call fib(${value}).`, 1, { depth: frames.length, calls });
        if (value <= 1) {
...
export const backtrackingSubsets = (input) => {
    ...
    const results = [];
    const frames = [];
    const snapshot = (explain, codeLine, marked = null) => {
        steps.push({
            scene: {
                array: {
                    values,
    ...
    steps.push({
        scene: {
            array: { values, states: values.map(() => 'visited'), caption: 'nums' },
...
export const backtrackingPermutations = (input) => {
    ...
    const results = [];
    const frames = [];
    const snapshot = (explain, codeLine) => {
        steps.push({
            scene: {
                array: {
                    values,
                    states: used.map((isUsed) => (isUsed ? 'match' : 'idle')),
 ...
 */
export const backtrackingNQueens = (input) => {
    ...
    const snapshot = (explain, codeLine, conflicts = []) => {
        steps.push({
            scene: {
