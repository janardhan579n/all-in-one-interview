...
export const balancedBrackets = (input) => {
    ...
    const stack = [];
    const expected = { '(': ')', '[': ']', '{': '}' };
    const scene = (index, state) => ({
        array: {
            values: characters,
            states: characters.map((_, i) => (i < index ? 'visited' : i === index ? state : 'idle')),
            caption: 'input',
            asText: true,
        },
 ...
 */
export const circularQueue = (input) => {
    ...
    for (const operation of operations) {
        if (operation.op === 'enqueue' && operation.value !== undefined) {
            ...
            steps.push({
                scene: scene('enqueue'),
                codeLine: 3,
                explain: tail === 0 && previousTail === capacity - 1
