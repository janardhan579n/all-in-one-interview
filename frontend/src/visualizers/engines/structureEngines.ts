import { cap, states, text, type CellState, type Engine, type GraphPanel, type Step } from '../types';

/**
 * Trie, union-find and monotonic stack.
 *
 * Each of these is a structure whose whole value is one idea, and in each case the idea is
 * invisible in the code and obvious in the picture: a trie shares prefixes, union-find flattens
 * a forest, and a monotonic stack discards values that can never be an answer again.
 */
 ...
 */
function layout(root: TrieNode): GraphPanel {
  const nodes: GraphPanel['nodes'] = [];
  const edges: GraphPanel['edges'] = [];
  let row = 0;

  const walk = (node: TrieNode, depth: number): number => {
...
function clearStates(node: TrieNode): void {
  node.state = undefined;
  node.children.forEach(clearStates);
}

export const trieOps: Engine = (input) => {
  const words = (Array.isArray(input.words) ? input.words : ['cat', 'car', 'card', 'dog']) as string[];
  ...
  for (const word of words) {
    ...
    for (const char of word) {
      ...
      if (existing) {
        ...
        node.state = 'inWindow';
        snapshot(
          `"${char}" already exists on this path — reuse it. This sharing is the entire reason a trie saves space.`,
          3,
          { word, char, reused: 'yes' },
        );
      } else {
        counter += 1;
    ...
    node.state = 'match';
    snapshot(
      `Mark the end of "${word}". Without this flag you could not tell a whole word from a prefix of one.`,
      5,
      { word, inserted: word },
    );
  }
  ...
  for (const char of lookup) {
    ...
    if (!node) {
      snapshot(
        `search("${lookup}") — no child for "${char}" after "${matched}". The word is not here, and we knew after ${matched.length + 1} characters.`,
        8,
        { search: lookup, result: 'absent' },
      );
      break;
    }
    matched += char;
  ...
  if (node) {
    ...
    snapshot(
      node.terminal
        ...
        : `"${lookup}" is only a prefix here, not a stored word — the node is not terminal.`,
      9,
      { search: lookup, result: node.terminal ? 'found' : 'prefix only' },
    );
  }
  ...
  for (const char of prefix) {
    ...
    if (cursor) cursor.state = 'inWindow';
  }
  if (cursor) {
    const collected: string[] = [];
    const walk = (current: TrieNode, built: string) => {
      current.state = current.state ?? 'visited';
      if (current.terminal) collected.push(built);
 ...
 */
export const unionFind: Engine = (input) => {
  const size = typeof input.size === 'number' ? input.size : 6;
  const operations = (Array.isArray(input.operations) ? input.operations : []) as UnionOperation[];
  const resolved: UnionOperation[] = operations.length
    ? operations
    : [
        { op: 'union', a: 0, b: 1 },
        { op: 'union', a: 2, b: 3 },
  ...
  const snapshot = (
    explain: string,
    codeLine: number,
    marked: Record<number, CellState> = {},
    vars: Record<string, string | number> = {},
  ) => {
    const cellStates = states(size);
    for (const [index, state] of Object.entries(marked)) cellStates[Number(index)] = state;
  ...
  const find = (node: number, codeLine: number): number => {
    const path: number[] = [];
    let current = node;
    ...
    if (path.length > 0) {
      snapshot(
        `find(${node}) walked ${path.length} link(s) up to root ${current}.`,
        codeLine,
        Object.fromEntries([...path.map((p) => [p, 'active' as CellState]), [current, 'match' as CellState]]),
        { find: node, root: current, hops: path.length },
      );
      for (const item of path) parent[item] = current;
      snapshot(
        `Path compression: every node on that walk now points straight at ${current}. The next find is one hop — this is why the amortised cost is effectively constant.`,
        codeLine + 1,
        Object.fromEntries([...path.map((p) => [p, 'visited' as CellState]), [current, 'match' as CellState]]),
  ...
  for (const operation of resolved) {
    ...
    if (operation.b === undefined) continue;

    const rootA = find(operation.a, 4);
    const rootB = find(operation.b, 4);
    if (rootA === rootB) {
      snapshot(
        `union(${operation.a}, ${operation.b}) — already in the same set (both reach ${rootA}). Nothing to do. In cycle detection, this is exactly the moment you have found a cycle.`,
        8,
        { [rootA]: 'match' },
      );
      continue;
    ...
    snapshot(
      `union(${operation.a}, ${operation.b}) — hang root ${loser} under root ${winner}. By rank, so the shallower tree goes underneath and depth barely grows.`,
      10,
      { [winner]: 'match', [loser]: 'target' },
      { merged: `${loser} → ${winner}` },
    );
  }
...
export const monotonicStack: Engine = (input) => {
  ...
  const snapshot = (explain: string, codeLine: number, index: number, vars: Record<string, string | number> = {}) => {
    const cellStates = states(values.length);
    for (const position of stack) cellStates[position] = 'inWindow';
    for (let i = 0; i < values.length; i++) if (result[i] !== -1) cellStates[i] = 'visited';
    if (index >= 0 && index < values.length) cellStates[index] = 'active';
    steps.push({
      scene: {
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    while (stack.length > 0 && values[stack[stack.length - 1]] < values[index]) {
      ...
      snapshot(
        `${values[index]} is greater than the waiting ${values[waiting]} — that is its answer. Pop it; it will never be asked about again.`,
        4,
        index,
        { resolved: `index ${waiting} → ${values[index]}` },
      );
    }

    stack.push(index);
    snapshot(
      `Push ${values[index]} and wait. The stack is now decreasing from bottom to top — anything smaller was discarded, because a bigger value standing in front of it makes it useless to everyone further right.`,
      6,
      index,
    );
  }
  ...
  snapshot(
    `Anything still on the stack has no greater element to its right — it stays -1. Each index was pushed once and popped at most once: ${values.length} pushes, ${pops} pops. That is the O(n), despite the nested loop.`,
    8,
    -1,
  );
