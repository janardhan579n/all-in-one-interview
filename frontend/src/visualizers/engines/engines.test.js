...
describe('heaps', () => {
    it('extracts in ascending order and keeps the heap property', () => {
        ...
        for (const step of steps) {
            const heap = step.scene.array?.values;
            if (!heap || heap.length < 2 || step.scene.secondaryArray)
                continue;
            for (let i = 1; i < heap.length; i++) {
                const parent = Math.floor((i - 1) / 2);
                // Snapshots taken mid-sift are allowed to violate it; the final one must not.
                if (step.done)
                    expect(heap[parent]).toBeLessThanOrEqual(heap[i]);
            }
        }
    });
    it('keeps the k largest with a min-heap of size k', () => {
        ...
        const final = steps[steps.length - 1];
        const heap = [...(final.scene.secondaryArray?.values ?? [])].sort((a, b) => b - a);
