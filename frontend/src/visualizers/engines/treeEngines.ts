import { cap, text, type CellState, type Engine, type Step, type TreeNodeView } from '../types';
...
function toView(node: RawTree | null | undefined, path: string, marks: Map<string, CellState>): TreeNodeView | null {
  if (!node) return null;
  const id = idFor(path);
  return {
    id,
 ...
 */
export const treeTraversal: Engine = (input) => {
  const tree = (input.tree as RawTree) ?? DEFAULT_TREE;
  const order = text(input, 'order', 'inorder') as 'preorder' | 'inorder' | 'postorder' | 'levelorder';
  const steps: Step[] = [];
  const marks = new Map<string, CellState>();
  const output: string[] = [];
  const frames: { label: string; detail?: string; state?: CellState }[] = [];

  const snapshot = (explain: string, codeLine: number | undefined, vars?: Record<string, string | number>) => {
    steps.push({
      scene: {
  ...
  const queue: { node: RawTree; path: string }[] = [];
  ...
  if (order === 'levelorder') {
    ...
    while (queue.length > 0) {
      ...
      for (let i = 0; i < levelSize; i++) {
        const { node, path } = queue.shift() as { node: RawTree; path: string };
  ...
  const walk = (node: RawTree | null | undefined, path: string) => {
    if (!node) {
 ...
 * asserted.
 */
export const bstOps: Engine = (input) => {
  const tree = structuredClone((input.tree as RawTree) ?? DEFAULT_TREE);
  ...
  const operations = (Array.isArray(input.operations) && input.operations.length > 0
    ? input.operations
    : [{ op: 'search', value: 7 }]) as {
    op: 'search' | 'insert';
    value: number;
  }[];
  const steps: Step[] = [];

  const markSubtree = (node: RawTree | null | undefined, path: string, marks: Map<string, CellState>, state: CellState) => {
    if (!node) return;
  ...
  const countNodes = (node: RawTree | null | undefined): number =>
    node ? 1 + countNodes(node.left) + countNodes(node.right) : 0;

  for (const operation of operations) {
    const marks = new Map<string, CellState>();
    let node: RawTree | null | undefined = tree;
    let path = '';
    let comparisons = 0;
    ...
    while (node) {
      ...
      const goLeft: boolean = operation.value < node.value;
      ...
      const next: RawTree | null | undefined = goLeft ? node.left : node.right;
      if (!next && operation.op === 'insert') {
        const fresh: RawTree = { value: operation.value };
