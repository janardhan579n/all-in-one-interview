...
export const arrayScanMax = (input) => {
    const values = numbers(input, 'array', [42, 17, 93, 8, 61, 93, 4]);
    const steps = [];
    let best = values[0];
    let bestIndex = 0;
    const snapshot = (cellStates) => ({
        ...
        if (operation.op === 'search' && operation.value !== undefined) {
            for (let i = 0; i < values.length; i++) {
                ...
                steps.push({
                    ...
                    explain: values[i] === operation.value
...
export const nestedLoopPairs = (input) => {
...
export const growthTable = (input) => {
    const sizes = numbers(input, 'sizes', [10, 100, 1000, 10000, 100000, 1000000]);
    const shapes = (Array.isArray(input.shapes) ? input.shapes : ['O(1)', 'O(log n)', 'O(n)', 'O(n log n)', 'O(n^2)']);
    const steps = [];
    const compute = (shape, n) => {
        switch (shape) {
            case 'O(1)': return 1;
    ...
    steps[steps.length - 1] = {
        ...steps[steps.length - 1],
        explain: 'Assuming ~10⁸ operations per second: O(n²) at n = 10⁶ would take about three hours. O(n log n) takes 0.2 seconds. Same problem, same machine.',
        done: true,
    };
    return cap(steps);
};
